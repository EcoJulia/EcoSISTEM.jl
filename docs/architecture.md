...
cell), `Supply{Precipitation}` (`L/day`), `Supply{CarbonFlux}` (`g/day`), `Supply{SurfaceArea}` (`m²`,
...
geographic grid gives `1.0 ° km⁻¹`, neither a length nor an angle, *silently*, with the eventual
