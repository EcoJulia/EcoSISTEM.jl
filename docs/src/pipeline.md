...
!!! warning "Currently unavailable"
    The `DataPipeline` integration does not work at present: `EcoSISTEM`'s extension for it cannot
    be loaded, because `DataPipeline` does not resolve against this package's current dependencies.
    The commands below are kept as a record of the intended workflow rather than as instructions
    that can be followed today.
