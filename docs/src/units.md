...
!!! note "`gsp` is the one where the role decides"
    ...
    **varies by cell**. So there is no fixed denominator, and `layerrate` deliberately leaves
    ...
    - as a **regime**, it stays `L m⁻²`. "How much water fell over the season" is what a
...
- **One axis spans rows that legitimately disagree.** WorldClim's `srad` is `kJ m⁻²`,
  CHELSA's `rsds` is `MJ m⁻²`. Both cells are right; without a declared scale the two are
  never reconciled.
...
That conversion is `cancel`: `kJ/m²/day × m² → kJ/day`, `L/m²/day × m² → L/day`,
`g/m²/day × m² → g/day`. Draw bigger cells and the condition is unchanged while the supply
...
So a `SurfaceSpec()` on a British National Grid study area is exactly `cellsize²` in every cell,
...
(`kJ/day/s`) rather than the per-area unit its spec was written in (`kJ/km²/day/s`): the spec
...
NPP is 3 g/m²/day and more is worse" — more is just more. Declaring no `Condition` unit is
...
| --- | --- | --- | --- |
| solar | `kJ/m²/day` | [`Supply`](@ref)`{SolarRadiation}`, `kJ/day` | [`Demand{SolarRadiation}`](@ref) |
| water | `mm/day` (= `L/m²/day`) | [`Supply`](@ref)`{Precipitation}`, `L/day` | [`Demand{Precipitation}`](@ref) |
| carbon | `g/m²/day` | [`Supply`](@ref)`{CarbonFlux}`, `g/day` | [`Demand{CarbonFlux}`](@ref) |
...
> A supply was built by looking at its value's dimension. `m/s` and `mm/day` are both `𝐋 𝐓⁻¹`, so a
> **wind-speed layer offered as a supply was silently built as a water supply** — 3 m/s over a 1 km²
> cell becoming 2.6 × 10¹⁴ L/day of rainfall.
