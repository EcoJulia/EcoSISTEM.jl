...
for i in 1:simDict["reps"]
    ...
    paramDict = Dict("numSpecies" => numSpecies, "numInvasive" => numInvasive, "numIndiv" => individuals, "reqs" => req, "opts" => opts, "vars" => vars, "birth" => birth_rates, "death" => death_rates, "s" => 1e-3, "boost" => 1.0, "kernel" => kernel, "totalK" => totalK, "bound" => NoBoundary())

    diver = zeros(length(divfuns), lensim, length(scenario))
    runsim!(diver, abenv, paramDict, simDict, i, "/home/claireh/Documents/Chapter3/Island/")
end
