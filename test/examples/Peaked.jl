...
numInvasive = 1; numSpecies = 10_000; grd = (50,50); req=(450000.0kJ/m^2, 192.0nm/m^2); individuals = 10^10; area = 200_000.0*km^2; totalK = (4.5e11kJ/km^2, 192.0mm/km^2)
...
scenario_names = ["Neutral1", "Neutral2", "GeneralistInvasive", "SpecialistInvasive", "TempIncrease1", "TempIncrease2", "TempIncrease3", "RandHabitatLoss", "ClustHabitatLoss"]
...
simDict = Dict("times" => 50years, "burnin" => 10year, "interval" => 1month, "timestep" => 1month, "scenarios" => scenario, "divfuns" => divfuns, "q" => q, "reps" => 55, "cacheInterval" => 1year, "scenario_names" => scenario_names)
...
for i in 1:simDict["reps"]
    ...
    paramDict = Dict("numSpecies" => numSpecies, "numInvasive" => numInvasive, "numIndiv" => individuals, "reqs" => req, "opts" => opts, "vars" => vars, "birth" => birth_rates, "death" => death_rates, "s" => 1e-3, "boost" => 1.0, "kernel" => kernel, "totalK" => totalK, "bound" => Torus())

    diver = zeros(length(divfuns), lensim, length(scenario))
    runsim!(diver, abenv, paramDict, simDict, i, "/media/storage/Chapter3/Continent/Peaked/")
end
