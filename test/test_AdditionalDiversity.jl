...
@testset "diversity" begin
    ...
    ## TEST meta_simpson
    @test meta_shannon(eco, 1.0)[!, :diversity] ==
          meta_shannon(eco, 2.0)[!, :diversity]
    @test meta_shannon(eco, 1.0)[!, :diversity][1] ≈ -sum(RelAb .* log.(RelAb))
    @test meta_shannon(eco, 1.0)[!, :diversity] ==
          log.(meta_gamma(makeunique(eco), 1.0)[!, :diversity])
    @test meta_shannon(eco, 2.0) == meta_shannon(makeunique(eco), 2.0)

    ## TEST meta_speciesrichness
    @test meta_speciesrichness(eco, 1.0)[!, :diversity] ==
          meta_speciesrichness(eco, 0.0)[!, :diversity]
    @test meta_speciesrichness(eco, 1.0)[!, :diversity][1] ≈ sum(Ab .> 0)
    @test meta_speciesrichness(eco, 1.0)[!, :diversity] ==
          meta_gamma(makeunique(eco), 0.0)[!, :diversity]
    @test meta_speciesrichness(eco, 0.0) ==
          meta_speciesrichness(makeunique(eco), 0.0)
    ...
    ## TEST geom_mean_abun
    @test geom_mean_abun(eco, 1.0)[!, :diversity] ==
          geom_mean_abun(eco, 0.0)[!, :diversity]
    @test geom_mean_abun(eco, 1.0)[!, :diversity][1] ≈
          exp.(sum(log.(mapslices(sum, eco.abundances.matrix, dims = 2) .+ 1)) ./
    ...
    @test pd(eco, 1.0)[!, :diversity][1] ≈
          sum(map(b -> getlength(eco.spplist.species.types.tree, b),
                  collect(getbranchnames(eco.spplist.species.types.tree)))) /
          mean(heightstoroot(eco.spplist.species.types.tree))
