...
@testset "SpeciesList" begin
    ...
    species = SpeciesTypes{typeof(sppl.species.traits),
                           typeof(sppl.species.requirement),
                           typeof(sppl.species.movement),
                           typeof(sppl.species.types)}(sppl.species.traits,
                                                       sppl.species.abun,
                                                       sppl.species.requirement,
                                                       sppl.species.types,
                                                       sppl.species.movement,
                                                       sppl.species.native)

    newsppl = SpeciesList{typeof(species), NoPathogen, typeof(sppl.params)}(species,
                                                                            NoPathogen(),
                                                                            sppl.params)
    @test newsppl.species.names == sppl.species.names

    # Test mass based species list
    @test_nowarn SpeciesList(numSpecies, numTraits, 10.0, 10.0, 0.5, 100.0km^2,
