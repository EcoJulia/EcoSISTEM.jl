...
@testset "Requirement and budget types" begin
    ...
    @test eltype(req) == [
        typeof(energy_vec1.energy[1]),
        typeof(energy_vec2.energy[1]),
        typeof(energy_vec3.energy[1])
    ]
    @test length(req) == length(energy_vec1.energy)
    ...
    @test EcoSISTEM._getbudget(bud, :b1) == bud1.matrix[:, :, 1]
    @test eltype(bud) == [
        typeof(bud1.matrix[1]),
        typeof(bud2.matrix[1]),
        typeof(bud3.matrix[1])
    ]
    @test EcoSISTEM._getavailableenergy(bud) ==
          [sum(bud1.matrix), sum(bud2.matrix), sum(bud3.matrix)]
end
