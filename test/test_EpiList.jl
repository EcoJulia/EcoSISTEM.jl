...
@testset "SpeciesList" begin
    ...
    @test_nowarn SpeciesList(traits, abun_v, abun_h, movement, transitions,
                             param)
    ...
    @test length(epilist.pathogens.names) ==
          length(epilist.pathogens.traits.mean)
    @test length(epilist.pathogens.names) ==
          length(epilist.pathogens.traits.var)
end
