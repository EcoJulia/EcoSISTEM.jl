...
@testset "GridHabitat" begin
    ...
    @testset "supply unit picks type; `within` honoured" begin
        ...
        # 1 g km^-2 day^-1 over a 1 km^2 cell is 1 g/day, absolutely
...
@testset "SolarRadiation: differently-scaled values reconcile to one canonical unit" begin
    # WorldClim's srad ships in kJ*m^-2*day^-1, CHELSA's rsds* in MJ*m^-2*day^-1 - this is the bug
    ...
    # the regime is canonicalised to kJ*m^-2*day^-1 regardless of the MJ input
...
# are both `𝐋𝐓^-1`, so a dimension-driven supply path turns `UniformSpec(3.0m/s, axis = WindSpeed)`
...
@testset "a non-resource axis cannot be built as a supply" begin
    ...
    #
    # The collision is not in `m/s` itself - that is `𝐋𝐓^-1`, while `VolumeFlow` is `𝐋^3𝐓^-1`. It
