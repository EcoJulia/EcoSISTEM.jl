...
@testset "SIS" begin
    ...
    for i in eachindex(grid_sizes)
        ...
        epilist = SpeciesList(traits, abun_v, abun_h, movement, transitions,
                              param)
    ...
    for j in 2:length(sumabuns)
        ...
        for i in 1:numclasses
            @test isapprox(sumabuns[j - 1][i, :], sumabuns[j][i, :],
                           rtol = 5e-2)
        end
    end
end
