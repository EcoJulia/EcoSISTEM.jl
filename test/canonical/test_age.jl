...
for i in eachindex(age_cats)
    ...
    epilist = SpeciesList(traits, abun_v, abun_h, movement, transitions, param, age_cats[i])
...
@testset " transition age" begin
    ...
    for i in eachindex(age_cats)
        ...
        numclasses = nrow(abun_h)
        numstates = sum(length.(abun_h.initial))
        cat_idx = reshape(1:(nrow(abun_h) * age_cats[i]), age_cats[i], nrow(abun_h))
        ...
        # Traits for match to environment (turned off currently through param choice, i.e. virus matches environment perfectly)
        traits = GaussTrait(fill(298.0K, numvirus), fill(0.1K, numvirus))
        epilist = SpeciesList(traits, abun_v, abun_h, movement, transitiondat, param, age_cats[i])
