...
@testset "SEI2HRD" begin
    ...
    param = (birth = birth, death = death,
             virus_growth = [virus_growth_asymp virus_growth_symp],
    ...
    # Run simulation
    abuns = zeros(Int64, size(epi.abundances.matrix, 1),
                  size(epi.abundances.matrix, 2), 366)
    times = 1year
    interval = 1day
    timestep = 1day
    @time simulate_record!(abuns, epi, times, interval, timestep;
                           save = do_save,
                           save_path = joinpath(save_path, "high_trans"))

    # Test everyone becomes infected and dies
    @test sum(abuns[1, :, 365]) == 0
    @test sum(abuns[end, :, 365]) ==
          (susceptible + new_symptomatic + new_asymptomatic)
    ...
    @time simulate_record!(abuns, epi, times, interval, timestep;
                           save = do_save,
                           save_path = joinpath(save_path, "low_trans"))
    ...
    @test sum(abuns[idx_dead, :, end]) ==
          (new_symptomatic + new_asymptomatic) + abun_h.initial[idx_dead]
