...
bioclim = ClimateRaster(WorldClim{BioClim},
                        AxisArray(Float64.(reshape(1:(length(blat) * length(blong) * 3),
                                                   length(blat), length(blong),
                                                   3)),
                                  Axis{:latitude}(blat),
...
@testset "separate lat/long steps (non-square grid)" begin
    ...
    @test extractvalues(20.0°, 10.0°, bioclim, 2) ==
          bioclim.array[atvalue(20.0°), atvalue(10.0°), 2]
end

@testset "coordinate bounds" begin
    @test_throws ErrorException extractvalues(120.0°, 10.0°, bioclim, 1)      # lat > 90°
    @test_throws ErrorException extractvalues(10.0°, 200.0°, bioclim, 1)      # long > 180°
    @test_throws ErrorException extractvalues([120.0°], [10.0°], bioclim, 1)
...
@testset "shape grid: scalar/vector × single/multi/all" begin
    ...
    @test extractvalues(10.0°, 10.0°, worldclim, 3month) ==
          worldclim.array[atvalue(10.0°), atvalue(10.0°), 3month]
    # scalar point, several slices -> length-M vector
    v = extractvalues(10.0°, 10.0°, worldclim, [3month, 6month])
    @test v isa AbstractVector && length(v) == 2
    # scalar point, all slices -> full third axis
    @test length(extractvalues(10.0°, 10.0°, worldclim)) == 12
    # vector points, single slice -> N×1
    m1 = extractvalues([10.0°, 20.0°], [10.0°, 20.0°], worldclim, 3month)
    @test size(m1) == (2, 1)
    # vector points, all slices -> N×M
    @test size(extractvalues([10.0°, 20.0°], [10.0°, 20.0°], worldclim)) ==
          (2, 12)
    # all 3 bioclim variables at N points
    @test size(extractvalues([20.0°, 40.0°], [10.0°, 20.0°], bioclim)) == (2, 3)
    # mismatched vector lengths error
    @test_throws ErrorException extractvalues([10.0°], [10.0°, 20.0°],
                                              worldclim)
end
...
@testset "year selection on an absolute time axis" begin
    ...
    @test_throws ErrorException extractvalues(10.0°, 10.0°, bioclim;
                                              year = 2000)
end
...
@testset "LatLong location forms (the core methods)" begin
    ...
    @test extractvalues(LatLong(10.0°, 10.0°), worldclim, 3month) ==
          extractvalues(10.0°, 10.0°, worldclim, 3month)
    # a vector of locations matches the separate-vectors form
    pts = [LatLong(10.0°, 10.0°), LatLong(20.0°, 20.0°)]
    @test extractvalues(pts, worldclim, 3month) ==
          extractvalues([10.0°, 20.0°], [10.0°, 20.0°], worldclim, 3month)
    @test size(extractvalues(pts, worldclim)) == (2, 12)
end
