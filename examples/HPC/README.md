...
⚠️ **None of these is run by the test suite.** `test/extras_examples.jl` executes only the top level
...
⚠️ The `#SBATCH` lines in those files name an account, partition and memory that are specific to one
