...
#     `L*m^-2`. Species are matched to it by a tolerance - some like a wet season, some a dry one.
#   * as a **supply** (a *resource*) it is water *per day*, a rate: `L*m^-2*d^-1`. Species consume it
...
# --- the environment ----------------------------------------------------------
# `gsp` appears in **both** arguments. As a regime it stays `L*m^-2`; as a supply it is silently
# rewritten to `gsp / gsl` and arrives as `L*d^-1` per cell. The names line up with the species'
...
heatmap(easting, northing, seasonwater,
        ...
        title = "gsp as a REGIME: growing-season water (L/m^2)",
        colorbar_title = "L/m^2", aspect_ratio = 1)
