...
function run_model(db::SQLite.DB, times::Unitful.Time, interval::Unitful.Time,
                   timestep::Unitful.Time; do_plot::Bool = false,
                   do_download::Bool = true, save::Bool = false,
                   savepath::String = pwd())

    # Download and read in population sizes for Scotland
    DataRegistryUtils.load_array!(db, "human/demographics/population/scotland",
                                  "/grid1km/age/persons";
                                  sql_alias = "human_demographics_population_scotland_grid1km_age_persons_arr")
    scotpop = get_3d_km_grid_axis_array(db, ["grid_x", "grid_y", "age_aggr"],
                                        "val", "scottish_population_view")
    ...
    abun_h = DataFrame([
                           (name = "Susceptible", type = Susceptible,
                            initial = fill(0, age_categories)),
                           (name = "Exposed", type = OtherDiseaseState,
                            initial = fill(0, age_categories)),
                           (name = "Asymptomatic", type = Infectious,
                            initial = fill(0, age_categories)),
                           (name = "Presymptomatic", type = Infectious,
                            initial = fill(0, age_categories)),
                           (name = "Symptomatic", type = Infectious,
                            initial = fill(0, age_categories)),
                           (name = "Hospitalised", type = OtherDiseaseState,
                            initial = fill(0, age_categories)),
                           (name = "Recovered", type = Removed,
                            initial = fill(0, age_categories)),
                           (name = "Dead", type = Removed,
                            initial = fill(0, age_categories))
                       ])
    ...
    total_pop = AxisArray(total_pop, AxisArrays.axes(scotpop)[1],
                          AxisArrays.axes(scotpop)[2])
    ...
    param_tab = read_table(db, "prob_hosp_and_cfr/data_for_scotland",
                           "cfr_byage")
    ...
    # Time exposed
    T_lat = days(read_estimate(db,
                               "human/infection/SARS-CoV-2/latent-period",
                               "latent-period",
                               key = "value",
                               data_type = Float64)[1] * Unitful.hr)

    # Time asymptomatic
    T_asym = days(read_estimate(db,
    ...
    T_presym = 1.5days
    # Time symptomatic
    T_sym = days(read_estimate(db,
                               "human/infection/SARS-CoV-2/infectious-duration",
                               "infectious-duration",
                               key = "value",
                               data_type = Float64)[1] * Unitful.hr) - T_presym
    # Time in hospital
    T_hosp = read_estimate(db,
                           "fixed-parameters/T_hos",
                           "T_hos",
                           key = "value",
                           data_type = Float64)[1] * days
    # Time to recovery if symptomatic
    T_rec = read_estimate(db,
    ...
    virus_growth_asymp = virus_growth_presymp = virus_growth_symp = fill(0.1 /
                                                                         day,
                                                                         age_categories)
    virus_decay = 1.0 / day
    ...
    param = (birth = birth_rates, death = death_rates,
             virus_growth = [virus_growth_asymp virus_growth_presymp virus_growth_symp],
             virus_decay = virus_decay, beta_force = beta_force,
             beta_env = beta_env, age_mixing = age_mixing)

    cat_idx = reshape(1:(nrow(abun_h) * age_categories), age_categories,
                      nrow(abun_h))
    transitiondat = DataFrame([
                                  (from = "Susceptible",
                                   from_id = cat_idx[:, 1], to = "Exposed",
                                   to_id = cat_idx[:, 2],
                                   prob = (env = beta_env, force = beta_force)),
                                  (from = "Exposed", from_id = cat_idx[:, 2],
                                   to = "Asymptomatic", to_id = cat_idx[:, 3],
                                   prob = mu_1),
                                  (from = "Exposed", from_id = cat_idx[:, 2],
                                   to = "Presymptomatic", to_id = cat_idx[:, 4],
                                   prob = mu_2),
                                  (from = "Presymptomatic",
                                   from_id = cat_idx[:, 4], to = "Symptomatic",
                                   to_id = cat_idx[:, 5], prob = mu_3),
                                  (from = "Symptomatic",
                                   from_id = cat_idx[:, 5], to = "Hospitalised",
                                   to_id = cat_idx[:, 6],
                                   prob = hospitalisation),
                                  (from = "Asymptomatic",
                                   from_id = cat_idx[:, 3], to = "Recovered",
                                   to_id = cat_idx[:, 7], prob = sigma_1),
                                  (from = "Symptomatic",
                                   from_id = cat_idx[:, 5], to = "Recovered",
                                   to_id = cat_idx[:, 7], prob = sigma_2),
                                  (from = "Hospitalised",
                                   from_id = cat_idx[:, 6], to = "Recovered",
                                   to_id = cat_idx[:, 7],
                                   prob = sigma_hospital),
                                  (from = "Symptomatic",
                                   from_id = cat_idx[:, 5], to = "Dead",
                                   to_id = cat_idx[:, 8], prob = death_home),
                                  (from = "Hospitalised",
                                   from_id = cat_idx[:, 6], to = "Dead",
                                   to_id = cat_idx[:, 8],
                                   prob = death_hospital)
                              ])
    ...
    movement_balance = (home = fill(0.5, numclasses * age_categories),
                        work = fill(0.5, numclasses * age_categories))
    ...
    from = active_cells
    to = sample(active_cells, weights(total_pop[active_cells]),
                length(active_cells))
    ...
    epilist = SpeciesList(traits, abun_v, abun_h, movement, transitiondat,
                          param, age_categories, movement_balance)
    ...
    @time epi = Ecosystem(epilist, epienv, rel, total_pop, UInt32(1),
                          initial_infected = initial_infecteds,
    ...
    for loc in epi.cache.ordered_active
        ...
        for age in 1:age_categories
            # Add Force
            addtransition!(epi.transitions,
                           ForceProduce(cat_idx[age, 3], loc,
                                        param.virus_growth[age, 1]))
            addtransition!(epi.transitions,
                           ForceProduce(cat_idx[age, 4], loc,
                                        param.virus_growth[age, 2]))
            addtransition!(epi.transitions,
                           ForceProduce(cat_idx[age, 5], loc,
                                        param.virus_growth[age, 3]))
            ...
            # Exposure
            addtransition!(epi.transitions,
                           Exposure(transitiondat[1, :from_id][age], loc,
                                    transitiondat[1, :to_id][age],
                                    transitiondat[1, :prob].force[age],
                                    transitiondat[1, :prob].env[age]))
            # Infected but asymptomatic
            addtransition!(epi.transitions,
                           Infection(transitiondat[2, :from_id][age], loc,
                                     transitiondat[2, :to_id][age],
                                     transitiondat[2, :prob][age]))
            # Infected but presymptomatic
            addtransition!(epi.transitions,
                           Infection(transitiondat[3, :from_id][age], loc,
                                     transitiondat[3, :to_id][age],
                                     transitiondat[3, :prob][age]))
            # Develop symptoms
            addtransition!(epi.transitions,
                           DevelopSymptoms(transitiondat[4, :from_id][age], loc,
                                           transitiondat[4, :to_id][age],
                                           transitiondat[4, :prob][age]))
            # Hospitalise
            addtransition!(epi.transitions,
                           Hospitalise(transitiondat[5, :from_id][age], loc,
                                       transitiondat[5, :to_id][age],
                                       transitiondat[5, :prob][age]))
            # Recover without symptoms
            addtransition!(epi.transitions,
                           Recovery(transitiondat[6, :from_id][age], loc,
                                    transitiondat[6, :to_id][age],
                                    transitiondat[6, :prob][age]))
            # Recover with symptoms
            addtransition!(epi.transitions,
                           Recovery(transitiondat[7, :from_id][age], loc,
                                    transitiondat[7, :to_id][age],
                                    transitiondat[7, :prob][age]))
            # Recover from hospital
            addtransition!(epi.transitions,
                           Recovery(transitiondat[8, :from_id][age], loc,
                                    transitiondat[8, :to_id][age],
                                    transitiondat[8, :prob][age]))
            # Die from home
            addtransition!(epi.transitions,
                           DeathFromInfection(transitiondat[9, :from_id][age],
                                              loc,
                                              transitiondat[9, :to_id][age],
                                              transitiondat[9, :prob][age]))
            # Die from hospital
            addtransition!(epi.transitions,
                           DeathFromInfection(transitiondat[10, :from_id][age],
                                              loc,
                                              transitiondat[10, :to_id][age],
                                              transitiondat[10, :prob][age]))
        end
    end

    # Populate susceptibles according to actual population spread
    scotpop = shrink_to_active(scotpop)
    reshaped_pop = reshape(scotpop[1:size(epienv.active, 1),
                                   1:size(epienv.active, 2), :],
                           size(epienv.active, 1) * size(epienv.active, 2),
                           size(scotpop, 3))'
    ...
    # Run simulation
    abuns = zeros(UInt32, size(epi.abundances.matrix, 1), N_cells,
                  floor(Int, times / timestep) + 1)
    @time simulate_record!(abuns, epi, times, interval, timestep, save = save,
                           save_path = savepath)
    ...
    if do_plot
        ...
        category_map = ("Susceptible" => cat_idx[:, 1],
                        "Exposed" => cat_idx[:, 2],
