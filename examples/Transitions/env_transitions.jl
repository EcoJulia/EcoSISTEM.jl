...
transitiondat = DataFrame([
                              (from = "Susceptible", from_id = cat_idx[:, 1],
                               to = "Exposed", to_id = cat_idx[:, 2],
                               prob = (env = beta_env, force = beta_force)),
                              (from = "Exposed", from_id = cat_idx[:, 2],
                               to = "Infected", to_id = cat_idx[:, 3],
                               prob = mu),
                              (from = "Infected", from_id = cat_idx[:, 3],
                               to = "Recovered", to_id = cat_idx[:, 4],
                               prob = sigma)
                          ])
    ...
    for age in 1:ages
        addtransition!(transitions,
                       ForceProduce(cat_idx[age, 3], loc,
                                    param.virus_growth[age]))
        addtransition!(transitions, ForceDisperse(cat_idx[age, 3], loc))
        addtransition!(transitions,
                       EnvExposure(transitiondat[1, :from_id][age], loc,
        ...
        addtransition!(transitions,
                       Recovery(transitiondat[3, :from_id][age], loc,
