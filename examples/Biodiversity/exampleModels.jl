...
reps = 100
areas = [10.0,20.0,50.0,100.0]
SR = zeros(Float64, length(areas), reps)
for r in reps
    for i in eachindex(areas)
        numSpecies = 100; grd = (10,10); req=(450000.0kJ/m^2, 192.0nm/m^2);
        individuals = 100_000_000; area = areas[i].*km^2;
        ...
        vars = rand(Uniform(2.0, 5.0), numSpecies) .* K
        opts = rand(Uniform(278.0, 308.0), numSpecies) .* K
        ...
        eco = Ecosystem(sppl, abenv, rel)
        simulate!(eco, times, timestep)
        SR[i, r] = meta_gamma(eco, 0.0)[1, :diversity]
    end
end

bar!(string.(areas), SR, grid = false, xlab = "Area (km²)",
ylab = "Total abundance", guidefontsize = 16,
tickfontsize= 16, titlefontsize=24, margin = 10.0*Plots.mm,
label = "", subplot = 4, title = "D", titleloc = :left,
left_margin = 20.0 *Plots.mm)
...
for i in eachindex(distances)
    ...
    Chapter3.newrun!(diver, abenv, paramDict, simDict, 1, "data/Examples/disperse")
end

endabun = mapslices(sum, JLD.load("data/Examples/disperse/cache/Neutral110001.jld", "abun"), dims = 1)[1,:]
endabun = reshape(endabun, 10, 10)

heatmap(grid = false, xlab = "Distance (km)", ylab = "Distance (km)", size = (1200, 800), guidefontsize = 12,tickfontsize= 12, titlefontsize=18, margin = 2.0*Plots.mm, legendfontsize = 12, label = "", layout = (@layout [a b; c d]), clim = (0, 1e6), link = :both)
for i in 1:4
    endabun = mapslices(sum, JLD.load("data/Examples/disperse/cache/Neutral$i"*"10001.jld", "abun"), dims = 1)[1,:]
    endabun = reshape(endabun, 10, 10)
    m = distances[i]
...
Plots.pdf("plots/DispersalSD.pdf")
