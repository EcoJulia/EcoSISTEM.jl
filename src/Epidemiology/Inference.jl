...
"""
function SIR_wrapper(grid_size::Tuple{Int64, Int64},
                     area::Unitful.Area{Float64}, params::NamedTuple,
                     runtimes::NamedTuple)
    times = runtimes.times
    interval = runtimes.interval
    Ncells = grid_size[1] * grid_size[2]
    numclasses = 4
    abuns = zeros(Int64, numclasses, Ncells,
...
function SIR_wrapper!(grid_size::Tuple{Int64, Int64},
                      area::Unitful.Area{Float64}, params::NamedTuple,
                      runtimes::NamedTuple, abuns::Array{Int64, 3})
    # Set up
    ...
    param = (birth = birth, death = death, virus_growth = params.virus_growth,
             virus_decay = params.virus_decay, beta_env = params.beta_env,
...
"""
function SEI3HRD_wrapper(grid_size::Tuple{Int64, Int64},
                         area::Unitful.Area{Float64}, params::NamedTuple,
                         runtimes::NamedTuple, num_ages::Int64 = 1)
    times = runtimes.times
    interval = runtimes.interval
    Ncells = grid_size[1] * grid_size[2]
    numclasses = 8
    abuns = zeros(Int64, numclasses * num_ages, Ncells,
                  convert(Int64, floor(times / interval)) + 1)
...
function SEI3HRD_wrapper!(grid_size::Tuple{Int64, Int64},
    ...
    abun_h = DataFrame([
                           (name = "Susceptible", type = Susceptible,
                            initial = fill(500_000 * Ncells, num_ages)),
                           (name = "Exposed", type = OtherDiseaseState,
                            initial = fill(0, num_ages)),
    ...
    transitions = DataFrame([
                                ...
                                (from = "Presymptomatic", to = "Symptomatic",
                                 prob = mu_3),
                                (from = "Symptomatic", to = "Hospitalised",
                                 prob = hospitalisation),
                                (from = "Asymptomatic", to = "Recovered",
                                 prob = sigma_1),
                                (from = "Symptomatic", to = "Recovered",
                                 prob = sigma_2),
                                (from = "Hospitalised", to = "Recovered",
                                 prob = sigma_hospital),
                                (from = "Symptomatic", to = "Dead",
                                 prob = death_home),
                                (from = "Hospitalised", to = "Dead",
                                 prob = death_hospital)
                            ])
    ...
    @assert length(params.beta_force) == length(params.beta_env) ==
            length(params.virus_growth_asymp) ==
            length(params.virus_growth_presymp) ==
            length(params.virus_growth_symp) == num_ages

    param = (birth = birth, death = death,
             virus_growth = [params.virus_growth_asymp params.virus_growth_presymp params.virus_growth_symp],
             virus_decay = params.virus_decay, beta_force = params.beta_force,
             beta_env = params.beta_env, age_mixing = age_mixing)
