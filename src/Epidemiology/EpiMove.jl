...
"""
mutable struct EpiMovement{MO1 <: AbstractMovement, MO2 <: AbstractMovement} <:
               AbstractMovement
    localmoves::MO1
    regionmoves::MO2
end

function EpiMovement(localkernels::Vector{K},
                     move_record::DataFrame) where {K <: AbstractKernel}
    localmoves = AlwaysMovement{K, NoBoundary}(localkernels, NoBoundary())
    regionmoves = LongDistance(move_record)
    return EpiMovement{typeof(localmoves), typeof(regionmoves)}(localmoves,
                                                                regionmoves)
end
...
function getdispersalvar(m::EpiMovement, sp::Int64)
    return (m.localmoves.kernels[sp].dist)^2 * pi / 4
end
