...
"""
env_bool(key, default = false) = haskey(ENV, key) ?
                                 lowercase(ENV[key]) ∉
                                 ["0", "", "false", "no"] : default
...
export SimpleRequirement, SizeRequirement, SolarRequirement, WaterRequirement,
       ...
       ReqCollection2, ReqCollection3, BudgetCollection2, BudgetCollection3
...
export GaussianKernel, LongTailKernel, BirthOnlyMovement, AlwaysMovement,
       NoMovement, getkernels, Torus, Cylinder, NoBoundary
...
mutable struct MPIEcosystem{MPIGL <: MPIGridLandscape, Part <: AbstractAbiotic,
               ...
               AbstractEcosystem{MPIGL, Part, SL, TR, SpeciesLookup, Cache}
    abundances::MPIGL
    spplist::SL
    abenv::Part
