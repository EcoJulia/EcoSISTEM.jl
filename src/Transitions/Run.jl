...
"""
function run_rule!(eco::Ecosystem, rule::S, timestep::Unitful.Time,
                   specialise = false) where {S <: AbstractStateTransition}
    if specialise
...
function run_rule!(eco::Ecosystem, rule::P, timestep::Unitful.Time,
                   specialise = false) where {P <: AbstractPlaceTransition}
    if specialise
        _run_rule!(eco, rule)
    else
...
function run_rule!(eco::Ecosystem, rule::SU, timestep::Unitful.Time,
                   specialise = false) where {SU <: AbstractSetUp}
    if specialise
        _run_rule!(eco, rule, timestep)
    else
...
function run_rule!(eco::Ecosystem, rule::WD, timestep::Unitful.Time,
                   specialise = false) where {WD <: AbstractWindDown}
    if specialise
        _run_rule!(eco, rule, timestep)
    else
...
@generated function run_generated!(eco::E, rule::AbstractStateTransition,
                                   timestep::Unitful.Time) where {
                                                                  StateTypes <:
    ...
    push!(generated.args,
          :(@error "Reached an unmatched rule type ($(typeof(rule)))"))
    return generated
end

@generated function run_generated!(eco::E, rule::AbstractSetUp,
                                   timestep::Unitful.Time) where {
                                                                  SetUpTypes <:
                                                                  AbstractSetUp,
                                                                  L, Part, SL,
...
@generated function run_generated!(eco::E,
                                   rule::AbstractPlaceTransition) where {
                                                                         PlaceTypes <:
                                                                         AbstractPlaceTransition,
                                                                         L,
                                                                         Part,
                                                                         SL,
                                                                         TR, LU,
...
@generated function run_generated!(eco::E, rule::AbstractWindDown,
                                   timestep::Unitful.Time) where {
                                                                  WindDownTypes <:
                                                                  AbstractWindDown,
                                                                  L, Part, SL,
...
"""
function update!(eco::AbstractEcosystem{L}, timestep::Unitful.Time, ::Nothing,
                 specialise = false) where {L <: EpiLandscape}
    return epi_update!(eco, timestep)
end
...
"""
function update!(eco::AbstractEcosystem{L}, timestep::Unitful.Time, ::Nothing,
                 specialise = false) where {L <: GridLandscape}
    return biodiversity_update!(eco, timestep)
end
...
at `save_path`.
"""
function simulate!(eco::AbstractEcosystem,
                   duration::Unitful.Time,
                   timestep::Unitful.Time;
                   save = false,
                   save_path = pwd(),
    ...
    save && JLSO.save(joinpath(save_path, "configuration.jlso"),
              :duration => duration,
              :timestep => timestep)
    ...
    return save && EcoSISTEM.save(joinpath(save_path, "final_system.jlso"), eco)
end
...
function simulate_record!(storage::AbstractArray,
    ...
    if save
        initialise_output(storage,
                          axes;
                          h5fn = joinpath(save_path, "abundances.h5"))
        update_output(eco.abundances.matrix,
                      counting;
                      h5fn = joinpath(save_path, "abundances.h5"))
    end
...
"""
function initialise_output(abuns::Array,
                           axes::NamedTuple;
                           h5fn = joinpath(pwd(), "abundances.h5"))
    # - sanity check
    if size(abuns) != length.(values(axes))
        throw(DimensionMismatch("abundances array size $(size(abuns)) does not match axes size " *
    ...
    h5open(h5fn, "w") do fid
        ...
        attributes(group)["Description"] = string("Contains the abundances for each compartment and geographic location ",
                                                  "through the simulation duration.")
        # - add data to the group
        # initialise abuns 3D array
        dset_abuns = create_dataset(group,
                                    "abuns",
...
"""
function update_output(abuns_t::Matrix{U},
                       timestep::Int;
                       h5fn = joinpath(pwd(), "abundances.h5")) where {U <:
                                                                       Integer}
    if !(isfile(h5fn) && ishdf5(h5fn))
        throw(ArgumentError("$(h5fn) does not exist or is not a valid HDF5 file"))
    end
    h5open(h5fn, "cw") do fid
        return fid["abundances"]["abuns"][:, :, timestep] = abuns_t
    end
end

function simulate!(eco::AbstractEcosystem, times::Unitful.Time,
                   timestep::Unitful.Time,
    ...
    for i in 1:length(time_seq)
        ...
        if mod(time_seq[i], cacheInterval) == 0year
            @save joinpath(cacheFolder,
                           scenario_name *
                           (@sprintf "%02d.jld" uconvert(NoUnits,
                                                         time_seq[i] /
