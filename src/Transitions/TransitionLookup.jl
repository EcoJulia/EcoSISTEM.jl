...
struct EpiLookup <: AbstractLookup
    ...
    function EpiLookup(locallookup::SparseMatrixCSC{Float64, Int32},
                       regionlookup::SparseMatrixCSC{Float64, Int32})
        all(0 .<= locallookup.nzval .<= 1) ||
            error("Home lookup values must be between 0 and 1")
        all(0 .<= regionlookup.nzval .<= 1) ||
            error("Work lookup values must be between 0 and 1")
...
function genlookups(epienv::GridEpiEnv, mov::AlwaysMovement)
    ...
    res = map((i, r, t) -> EcoSISTEM.genlookups(i, grid_locs, xys, grid_size, r,
                                                t, epienv), grid_locs, relsize,
              thresh)
...
function genlookups(from::Int64, to::Vector{Int64},
                    xys::Array{Tuple{Int64, Int64}, 1}, grid_size::Float64,
                    relsize::Float64, thresh::Float64, epienv::GridEpiEnv)
    x, y = xys[to .== from][1]
    maxX = ceil(Int64, x + 1 / relsize)
    minX = ceil(Int64, x - 1 / relsize)
    maxY = floor(Int64, y + 1 / relsize)
    minY = floor(Int64, y - 1 / relsize)
    keep = [(i[1] <= maxX) & (i[2] <= maxY) & (i[1] >= minX) & (i[2] >= minY)
            for i in xys]
    to = to[keep]
    probs = [_lookup((x = x, y = y), (x = i[1], y = i[2]), relsize,
                     _gaussian_disperse) for i in xys[keep]]
    keep = probs .> thresh
    probs = probs[keep]
    probs ./= sum(probs)
...
function _lookup(from::NamedTuple, to::NamedTuple, relSquareSize::Float64,
                 dispersalfn::F) where {F <: Function}
    return calc_prob = hcubature(r -> dispersalfn(r),
                                 [
                                     from.y * relSquareSize - relSquareSize,
                                     from.x * relSquareSize - relSquareSize,
                                     to.y * relSquareSize - relSquareSize,
                                     to.x * relSquareSize - relSquareSize
                                 ],
                                 [
                                     from.y * relSquareSize,
                                     from.x * relSquareSize,
                                     to.y * relSquareSize,
                                     to.x * relSquareSize
                                 ],
