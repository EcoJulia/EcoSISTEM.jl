...
function create_cache(sppl::SpeciesList, ml::GridLandscape)
    ...
    totalE = zeros(Float64,
                   (size(ml.matrix, 2),
                    numrequirements(typeof(sppl.species.requirement))))
...
function create_cache(sppl::SpeciesList{SpeciesTypes{TR, R, MO, T}},
                      ml::GridLandscape) where {TR, R, MO <: BirthOnlyMovement,
                                                T}
    nm = zeros(Int64, size(ml.matrix))
...
function tematch(sppl::SpeciesList, abenv::AbstractAbiotic)
    return (eltype(sppl.species.traits) == eltype(abenv.habitat)) &&
...
function trmatch(sppl::SpeciesList, traitrel::AbstractTraitRelationship)
    return eltype(sppl.species.traits) == eltype(traitrel) &&
...
"""
abstract type AbstractEcosystem{L <: AbstractLandscape,
                                Part <: AbstractPartition,
                                SL <: AbstractSpeciesList,
                                TR <: AbstractTraitRelationship,
                                LU <: AbstractLookup, C <: AbstractCache} <:
...
"""
mutable struct Ecosystem{L <: AbstractLandscape, Part <: AbstractPartition,
                         SL <: AbstractSpeciesList,
                         TR <: AbstractTraitRelationship, LU <: AbstractLookup,
                         C <: AbstractCache, TL <: TransitionList} <:
               AbstractEcosystem{L, Part, SL, TR, LU, C}
    abundances::L
    spplist::SL
    abenv::Part
...
function Ecosystem(popfun::F, spplist::SpeciesList{T, Req},
                   ...
                   transitions::Union{Nothing, TransitionList} = nothing) where {
                                                                                 F <:
                                                                                 Function,
                                                                                 T,
                                                                                 Req
    ...
    lookup = SpeciesLookup(collect(map(k -> genlookups(abenv.habitat, k),
                                       getkernels(spplist.species.movement))))
    ...
    return Ecosystem{typeof(ml), typeof(abenv), typeof(spplist), typeof(rel),
                     typeof(lookup), typeof(cache), typeof(tl)}(ml, spplist,
                                                                abenv,
                                                                missing, rel,
                                                                lookup, cache,
                                                                transitions)
...
"""
function Ecosystem(abundances::EpiLandscape{U, VecRNGType}, epilist::EL,
                   epienv::EE,
                   ordinariness::Union{Matrix{Float64}, Missing},
                   relationship::ER, lookup::EpiLookup,
                   vm::Array{Float64, 2}, initial_infected::Int64, valid::Bool,
                   transitions::Union{Nothing, TransitionList}) where {
                                                                       U <:
                                                                       Integer,
                                                                       VecRNGType <:
    ...
    return Ecosystem{typeof(abundances), typeof(epienv), typeof(epilist),
                     typeof(relationship), typeof(lookup), typeof(cache),
                     typeof(tl)}(abundances, epilist, epienv, ordinariness,
                                 relationship, lookup, cache, transitions)
end
...
function Ecosystem(popfun::F, epilist::SpeciesList, epienv::GridEpiEnv,
                   rel::AbstractTraitRelationship, intnum::U;
                   initial_infected = 0,
                   rngtype::Type{R} = Random.MersenneTwister,
                   transitions::Union{Nothing, TransitionList} = nothing) where {
                                                                                 F <:
    ...
    region_lookup = genlookups(epienv, getregion(epilist.species.movement),
                               initial_pop)
    lookup = EpiLookup(local_lookup, region_lookup)
    vm = zeros(Float64, size(ml.matrix))
    return Ecosystem(ml, epilist, epienv, missing, rel, lookup, vm,
                     initial_infected, false, transitions)
end
...
from the start abundances in `spplist`.
"""
function Ecosystem(epilist::SpeciesList, epienv::GridEpiEnv,
                   rel::AbstractTraitRelationship,
                   intnum::U = Int64(1); initial_infected = 0,
                   rngtype::Type{R} = Random.MersenneTwister,
                   transitions = nothing) where {U <: Integer,
                                                 R <: Random.AbstractRNG}
    return Ecosystem(populate!, epilist, epienv, rel, intnum,
                     initial_infected = initial_infected, rngtype = rngtype,
...
"""
function Base.isapprox(eco_1::AbstractEcosystem, eco_2::AbstractEcosystem;
                       kwargs...)
...
mutable struct CachedEcosystem{Part <: AbstractAbiotic, SL <: SpeciesList,
                               TR <: AbstractTraitRelationship} <:
               AbstractEcosystem{CachedGridLandscape, Part, SL, TR,
                                 SpeciesLookup, Cache}
