...
getdispersaldist(m::BirthOnlyMovement, sp::Int64) = m.kernels[sp].dist
function getdispersalvar(m::BirthOnlyMovement, sp::Int64)
    return (m.kernels[sp].dist)^2 * pi / 4
end
...
getdispersaldist(m::NoMovement, sp::Int64) = error("No movement takes place")
getdispersalvar(m::NoMovement, sp::Int64) = error("No movement takes place")
