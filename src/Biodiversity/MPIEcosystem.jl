...
function EcoSISTEM.MPIEcosystem(abundances::MPIGL,
                                ...
                                ordinariness::Union{Matrix{Float64},
                                                    Missing}, relationship::TR,
                                lookup::Vector{EcoSISTEM.Lookup},
                                sppcounts::Vector, firstsp::Int64,
                                sccounts::Vector,
                                firstsc::Int64,
                                cache::EcoSISTEM.Cache) where {
                                                               MPIGL <:
                                                               MPIGridLandscape,
                                                               Part <:
...
"""
function EcoSISTEM.MPIEcosystem(popfun::F,
                                spplist::SpeciesList{SpeciesTypes{T, Req, MO,
                                                                  TY}, P, PA},
                                abenv::GridAbioticEnv,
                                rel::EcoSISTEM.AbstractTraitRelationship) where {
                                                                                 F <:
                                                                                 Function,
                                                                                 T,
                                                                                 Req,
    ...
    lookup_tab = collect(map(k -> EcoSISTEM.genlookups(abenv.habitat, k),
                             @view getkernels(spplist.species.movement)[rankspp]))
    nm = zeros(Int64, (sppcounts[rank + 1], numsc))
    totalE = zeros(Float64, (numsc, EcoSISTEM.numrequirements(Req)))
    return MPIEcosystem{typeof(ml), typeof(abenv),
                        typeof(spplist), typeof(rel)}(ml, spplist, abenv,
                                                      missing, rel,
                                                      lookup_tab, sppcounts,
                                                      firstsp, sccounts,
                                                      firstsc,
                                                      EcoSISTEM.Cache(nm,
                                                                      totalE,
...
function Diversity.API._getabundance(eco::MPIEcosystem, raw::Bool)
    if raw
        ...
        return _calcabundance(Diversity.API._gettypes(eco),
...
function Diversity.API._calcordinariness(eco::MPIEcosystem)
    ...
    return Diversity.API._calcsimilarity(eco.spplist.species.types,
                                         one(eltype(relab)))[sp_rng, sp_rng] *
           relab
end
