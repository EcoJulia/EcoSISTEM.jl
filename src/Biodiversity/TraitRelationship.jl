...
function eltype(::Trapeze{TR}) where {TR}
    return TR
end
...
function eltype(::Unif{TR}) where {TR}
    return TR
end
...
iscontinuous(::Match{TR}) where {TR} = false
function eltype(::Match{TR}) where {TR}
    return TR
end
...
function eltype(::LCmatch{TR}) where {TR}
    return TR
end
...
function eltype(::NoRelContinuous{TR}) where {TR}
    return TR
end
...
"""
function combineTR(::Union{multiplicativeTR2, multiplicativeTR3})
    return *
end
function combineTR(::Union{additiveTR2, additiveTR3})
    return +
end
