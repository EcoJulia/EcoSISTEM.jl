...
"""
mutable struct ContinuousHab{C <: Number, L <: Unitful.Length} <:
               AbstractHabitat{C}
    matrix::Matrix{C}
    size::L
...
mutable struct ContinuousTimeHab{C <: Number, M <: AbstractArray{C, 3},
                                 L <: Unitful.Length} <: AbstractHabitat{C}
    matrix::M
    time::Int64
    size::L
    change::HabitatUpdate
end
@recipe function f(H::ContinuousTimeHab{C, M, L},
                   time::Int64) where {C, M <: AbstractArray{C, 3}, L}
    h = ustrip.(H.matrix)
    seriestype := :heatmap
    grid --> false
    aspect_ratio --> 1
...
function iscontinuous(hab::ContinuousTimeHab{C, M, L}) where {C,
                                                              M <:
                                                              AbstractArray{C,
                                                                            3},
                                                              L}
...
function eltype(hab::ContinuousTimeHab{C, M, L}) where {C,
                                                        M <:
                                                        AbstractArray{C, 3}, L}
    return C
end
...
function _getdimension(hab::H) where {H <: Union{DiscreteHab, ContinuousHab,
                                            ContinuousTimeHab}}
    return (size(hab.matrix, 1), size(hab.matrix, 2))
end
function _getsize(hab::H) where {H <: Union{DiscreteHab, ContinuousHab,
                                       ContinuousTimeHab}}
...
function size(hab::H,
              d) where {H <:
                        Union{DiscreteHab, ContinuousHab, ContinuousTimeHab}}
    return size(hab.matrix, d)
end

function _getgridsize(hab::H) where {H <: Union{DiscreteHab, ContinuousHab,
                                           ContinuousTimeHab}}
    return hab.size
end
...
function _getdimension(hab::H) where {H <: Union{HabitatCollection2,
                                            HabitatCollection3}}
    return (size(hab.h1.matrix, 1), size(hab.h2.matrix, 2))
end
function _getsize(hab::H) where {H <:
                                 Union{HabitatCollection2, HabitatCollection3}}
    return _getsize(hab.h1)
end
function size(hab::H,
              d) where {H <: Union{HabitatCollection2, HabitatCollection3}}
    return size(hab.h1, d)
end

function _getgridsize(hab::H) where {H <: Union{HabitatCollection2,
                                           HabitatCollection3}}
