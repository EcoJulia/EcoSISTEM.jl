...
function get_neighbours(mat::Matrix, x_coord::Int64, y_coord::Int64,
    ...
    if chess == 4
        neighbour_vec = [x_coord y_coord-1; x_coord y_coord+1;
                         x_coord-1 y_coord;
                         x_coord+1 y_coord]
    ...
    Threads.@threads for j in 1:spp
        ...
        for i in 1:dims
            ...
            if eco.abenv.active[x, y] && (eco.cache.totalE[i, 1] > 0)
                ...
                move!(eco, eco.spplist.species.movement, i, j,
...
"""
function update_energy_usage!(eco::AbstractEcosystem{A, B,
                                                     SpeciesList{SpeciesTypes{C,
                                                                              Req,
                                                                              D,
                                                                              E},
                                                                 F, G}, H, I,
...
function energy_adjustment(eco::AbstractEcosystem, bud::AbstractBudget,
    ...
    if typeof(eco.spplist.params) <: NoGrowth
        ...
        # Get energy budgets of species in square
        ϵ̄ = eco.spplist.species.requirement.energy[sp] *
             eco.spplist.species.requirement.exchange_rate
...
function energy_adjustment(eco::AbstractEcosystem, bud::BudgetCollection2,
    ...
    # Get energy budgets of species in square
    ϵ̄1 = eco.spplist.species.requirement.r1.energy[sp] *
          eco.spplist.species.requirement.r1.exchange_rate
    ϵ̄2 = eco.spplist.species.requirement.r2.energy[sp] *
          eco.spplist.species.requirement.r2.exchange_rate
...
function move!(eco::AbstractEcosystem, ::AlwaysMovement, i::Int64, sp::Int64,
    ...
    calc_lookup_moves!(getboundary(eco.spplist.species.movement), x, y, sp, eco,
                       full_abun)
...
function populate!(ml::GridLandscape, spplist::SpeciesList,
    ...
    # Loop through species
    for i in eachindex(spplist.species.abun)
        rand!(Multinomial(spplist.species.abun[i], B ./ sum(B)),
              (@view ml.matrix[i, :]))
    end
end
...
function repopulate!(eco::Ecosystem)
    ...
    eco.spplist.species.abun = rand(Multinomial(sum(eco.spplist.species.abun),
                                                length(eco.spplist.species.abun)))
...
function traitpopulate!(ml::GridLandscape, spplist::SpeciesList,
    ...
    probabilities = [_traitfun(abenv.habitat, spplist.species.traits, rel, i,
                               sp) for i in 1:numsquares, sp in 1:numspp]
    # Loop through species
    for i in eachindex(spplist.species.abun)
