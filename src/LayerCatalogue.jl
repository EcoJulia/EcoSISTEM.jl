...
`publishedscale` is the factor a **provider** ships the layer at when that is not the documented one
— `100` for `bio4` and `10 000` for the fourteen HabitatHeterogeneity layers, blank everywhere else.
...
# either — `WindSpeed` is `m s⁻¹`, dimensionally `𝐋𝐓⁻¹`, and yet honestly `instantaneous` — so the
...
# Unitful write the same thing differently — the CSV has `𝐋 * 𝐓^-1`, Unitful shows `𝐋 𝐓⁻¹` — so a
...
# direction — and a silent factor of 10⁴ is exactly what this whole area exists to prevent. So each
...
# Sampled rather than swept — these rasters run to ~10⁹ cells and the check runs on every read.
...
# (𝐋) while `Precipitation`'s canonical unit is `mm/day` (𝐋𝐓⁻¹) — dimensionally different, yet both
...
function _checkaxisunit(u, period, axis, code)
    ...
    # and it is wrong. A catalogue row states an **areal flux density** (`npp` is `g·m⁻²`), while a
    # Resource-role canonical unit is a **per-cell rate** (`g·day⁻¹`); the two differ by the cell area
...
# WorldClim's `kJ·m⁻²` with CHELSA's `MJ·m⁻²` on one axis is the point, and a per-dataset grouping
