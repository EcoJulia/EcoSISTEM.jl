    ...
    map(aa_ax[3]) do time
        for x in aa_ax[1]
            for y in aa_ax[2]
                ...
                (rescale * y - (rescale - 1)):(rescale * y), time] .= aa[x, y,
                                                                         time]
            end
        end
    end
...
function downresolution(era::ERA, rescale::Int64;
                        fn::F = mean) where {F <: Function}
    ...
    map(1:grid[3]) do tm
        for x in Base.axes(array, 1)
            for y in Base.axes(array, 2)
                xcoords = filter(x -> x .<= size(aa, 1),
                                 (rescale * x - (rescale - 1)):(rescale * x))
                ycoords = filter(y -> y .<= size(aa, 2),
...
function downresolution!(resized_array::Array{T, 3}, array::Matrix{T},
                         dim::Int64, rescale::Int64, fn) where {T}
