...
const UNITDICT = Dict("K" => K, "m" => m, "J m**-2" => J / m^2,
                      "m**3 m**-3" => m^3, "kg m-2 s-1" => kg * m^-2 * s^-1,
                      "1" => 1, nothing => W * m^-2, "d" => day)
...
function readMet_raw(dir::String, param::String)
    ...
    uk = AxisArray(unitarray, Axis{:longitude}(lon * °),
                   Axis{:latitude}(lat * °))
    return uk
end
...
"""
function readMet_raw(dir::String, file::String, param::String, dim::Vector{T},
                     process::Bool = true) where {T <: Unitful.Time}
...
function readMet(dir::String, param::String)
    ...
    return AxisArray(climatearray .* units, Axis{:x}(x), Axis{:y}(y),
                     Axis{:time}(times))
end
