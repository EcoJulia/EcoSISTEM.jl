...
rates, `birth` and `death`, as well as how these are altered by resource
availability. `l` represents the longevity of species based on their resource
demands and `s` is the survival of species dependent on how well their
tolerances reflect the environment.
"""
...
Parameter type that holds information on a population's birth and death rates,
`birth` and `death`, specifically populations where all species have the same
information. `l` represents the longevity of species based on their resource
demands and `s` is the survival of species dependent on how well their
tolerances reflect the environment. Finally `boost` is used to manipulate how much
...
Function that takes demographic parameters from type [`EqualPop`](@ref) and
converts them into type [`PopGrowth`](@ref) based on the number of species
(`numspp`).
"""
