...
# humidity and L·m⁻² fluxes/stocks without conflict; each leaf (or sub-group) declares its own.
...
# `SnowWaterEquivalent`, `SiteWaterBalance`, `GrowingSeasonPrecipitation` — genuinely are `L·m⁻²`, a
...
# `L·m⁻²·day⁻¹`, and `_readdivisors` — which asks `layerrate` whether the unit changed — divides each
# slice by *its own* month. Unit and values move together, or not at all.
# Stated in `mm/day` rather than `L/(m²·day)` to match [`Precipitation`](@ref): `cmi` is literally
# `prec − pet`, so the three should not print in two different spellings of the same unit. They are
# exactly equal (1 L·m⁻² ≡ 1 mm), so this is a display choice, not a conversion.
...
"""    SnowWaterEquivalent <: WaterAxis — snow water equivalent, the liquid-water amount of the snowpack (BioClimPlus swe; L·m⁻²). """
...
"""    SiteWaterBalance <: WaterAxis — cumulative site water balance over the year, capped by soil water-holding capacity (BioClimPlus swb; L·m⁻²). """
...
"""    GrowingSeasonPrecipitation <: WaterAxis — precipitation accumulated over the growing season (BioClimPlus gsp; L·m⁻²). """
...
# Condition one, which is what makes `npp` read as `g·m⁻²·day⁻¹` rather than its annual total.
"""    CarbonFlux <: CarbonAxis — net primary productivity, a carbon flux (BioClimPlus npp; g·m⁻²·day⁻¹). """
...
#
# **Areal, not per-cell**, exactly like solar and water: a layer holds a **fraction** (m²/m², so
# dimensionless), and `cancel` multiplies by the cell's own area to give the per-cell supply in m².
...
# a dimension: surface and canopy are areas (m²/m² → m²), soil is a volume (m³/m² → m³). A single
...
axis' **canonical unit**. Either end is `nothing` where no bound applies, and `(nothing, nothing)`
— the default — means the axis is unbounded in both directions.
...
`balance` row is sign-indefinite — a difference of two quantities, which may fall either side of zero
— and is *therefore* not supply-eligible either. The two are one fact, and a testset asserts that the
...
# defect the whole interface above exists to remove: `m/s` and `mm/day` are both `𝐋𝐓⁻¹`, so choosing a
